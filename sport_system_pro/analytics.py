...
def summarize(records: list[SportRecord], target_minutes: int = 150) -> dict[str, object]:
    sport_count = Counter(r.sport_type for r in records)
...
def leaderboard(records: list[SportRecord]) -> list[tuple[int, str, str, int, float, int]]:
    ...
    for r in records:
        ...
        item["points"] = int(item["points"]) + max(1, r.duration // 10) + (5 if r.duration >= 30 else 0)
    rows = []
    for item in bucket.values():
        rows.append((str(item["name"]), str(item["class"]), int(item["duration"]), round(float(item["calories"]), 1), int(item["points"])))
...
def diet_advice(profile: HealthProfile, recent_calories: float) -> str:
    ...
    if profile.fitness_goal == "减脂" or bmi >= 24:
        return f"饮食建议：今日运动约消耗 {recent_calories:.0f} kcal，可适当增加鸡胸肉/鸡蛋/豆制品，主食减量但不要完全不吃。"
    if profile.fitness_goal == "增肌":
        return f"饮食建议：今日运动约消耗 {recent_calories:.0f} kcal，训练后 1-2 小时内补充碳水和蛋白，避免只吃零食。"
    return f"饮食建议：今日运动约消耗 {recent_calories:.0f} kcal，三餐保持蔬菜、蛋白和主食均衡即可。"


def predict_test_score(records: list[SportRecord], tests: list[PhysicalTestRecord]) -> str:
    recent_minutes = sum(r.duration for r in records if r.status == "已通过")
